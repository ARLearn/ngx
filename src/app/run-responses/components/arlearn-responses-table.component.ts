        ...
        <div class="answers" *ngIf="editMessage$ | async as editMessage">
            <ng-container *ngIf="isMediaQuestion(editMessage.type); else answers">
                ...
                <ng-container *ngIf="isAudioQuestion(editMessage.type) && selectedScreen && players && players.length">
                    <app-audio-gallery class="w-100" [responses]="data" [user]="selectedUser"
                                       (onLoad)="onGalleryLoading($event)"></app-audio-gallery>
                </ng-container>
                <ng-container *ngIf="isVideoQuestion(editMessage.type) && selectedScreen && players && players.length">
                    <app-video-gallery class="w-100" [responses]="data" [user]="selectedUser"
                                       (onLoad)="onGalleryLoading($event)"></app-video-gallery>
                </ng-container>
                <ng-container *ngIf="isTextQuestion(editMessage.type) && selectedScreen">
                    <app-text-questions-gallery class="w-100" [responses]="data" [user]="selectedUser"
                                                (onLoad)="onGalleryLoading($event)"></app-text-questions-gallery>
                </ng-container>
            </ng-container>
