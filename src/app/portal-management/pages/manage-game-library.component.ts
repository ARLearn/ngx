...
import { GetPortalGamesRequestAction } from '../store/portal-games.actions';
import { getPortalGames } from '../store/portal-games.selector';
  ...
  template: `
    ...
    <div class="game-library maxwidth">
      <div class="mb-3 d-flex align-items-center justify-content-between">
        ...
        <div>
            <button mat-button [matMenuTriggerFor]="orgMenu" class="pr-0">Rating <mat-icon>arrow_drop_down</mat-icon></button>
      ...
      <ng-container *ngIf="data$ | async as dataSource">
        <table mat-table [dataSource]="dataSource" class="game-library-table">
          ...
          <ng-container matColumnDef="country">
            <th mat-header-cell *matHeaderCellDef></th>
            <td mat-cell *matCellDef="let element" class="country"> {{element.country}} </td>
          </ng-container>

          <ng-container matColumnDef="private">
            <th mat-header-cell *matHeaderCellDef></th>
            <td mat-cell *matCellDef="let element" class="private center"> <mat-icon class="table-icon" *ngIf="element.private">lock</mat-icon> </td>
          </ng-container>

          <ng-container matColumnDef="rate">
            <th mat-header-cell *matHeaderCellDef></th>
            <td mat-cell *matCellDef="let element" class="rate">
              <span> <mat-icon class="table-icon" color="primary">star</mat-icon> {{ element.rate }} </span>
              <span class="reviews"> ({{ element.reviews }}) </span>
            </td>
          </ng-container>

          <tr mat-header-row *matHeaderRowDef="displayedColumns"></tr>
          <tr mat-row *matRowDef="let row; columns: displayedColumns;" class="game-row" [routerLink]="'/portal/root/portal/' + row.id"></tr>
        </table>
      </ng-container>
    </div>
  `,
...
export class ManageGameLibraryComponent implements OnInit {
  public data$ = this.store.select(getPortalGames);
  public displayedColumns: string[] = ['icon', 'title', 'date', 'category', 'country', 'private', 'rate'];
